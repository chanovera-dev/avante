...
if (!function_exists('avante_collect_image_ids_from_blocks')) {
     ...
     */
    function avante_collect_image_ids_from_blocks($blocks)
    {
        $ids = array();
        ...
        foreach ($blocks as $block) {
                    ...
                    if (!empty($block['innerBlocks'])) {
                        foreach ($block['innerBlocks'] as $ib) {
                            if (!empty($ib['blockName']) && $ib['blockName'] === 'core/image') {
                                ...
                                // Recursive
                                $ids = array_merge($ids, avante_collect_image_ids_from_blocks(array($ib)));
                            }
                        }
                    }
            ...
            // --- 3) General recursion ---
            if (!empty($block['innerBlocks'])) {
                $ids = array_merge($ids, avante_collect_image_ids_from_blocks($block['innerBlocks']));
            }
        }
...
if (!function_exists('avante_extract_gallery_images')) {
    ...
    function avante_extract_gallery_images($post_id)
    {
        ...
        if (function_exists('parse_blocks') && !empty($content)) {
            ...
            $image_ids = avante_collect_image_ids_from_blocks($blocks);
        }
