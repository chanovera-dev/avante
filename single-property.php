...
while ( have_posts() ) : the_post();
    ...
    $full_metadata = avante_get_full_property_metadata();
?>
...
<main id="main" class="site-main" role="main">
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
                    ...
                    <div class="property--metadata">
                        ...
                        <ul class="property---metadata--list">
                            <?php
                                ...
                                echo '<li>' . avante_get_icon('id') . 'ID: ' . esc_html($id) . '</li>';
                                echo '<li>' . avante_get_icon('location') . esc_html($location) . '</li>';
                                
                                // Display construction size if available
                                if (!empty($construction) && $construction != 0) {
                                    echo '<li>' . avante_get_icon('construction') . format_numeric($construction) . ' m² de construcción</li>';
                                }
                                
                                // Display lot size if available
                                if (!empty($lot) && $lot != 0) {
                                    echo '<li class="lot">' . avante_get_icon('lot') . format_numeric($lot) . ' m² de terreno</li>';
                                }
                            ?>
                        </ul>
                    </div>
        ...
        <section class="block property--details">
            <div class="content">
                <h2 class="title-section"><?php esc_html_e( 'Detalles de la propiedad', 'avante' ); ?></h2>
            </div>
            <div class="content details">
                <div class="property--metadata">
                    <ul class="property--metadata--list">
                        <?php avante_render_full_property_metadata(); ?>
                    </ul>                
                </div>
                <div class="is-layout-constrained">
                    <?php the_content(); ?>
        ...
        <section class="block posts--body container--related-posts">
            <div class="content related-posts--title">
                <h2 class="title-section"><?php echo esc_html_e( 'Propiedades cercanas', 'avante' ); ?></h2>
            </div>
