document.addEventListener("DOMContentLoaded", function () {
    ...
    wrappers.forEach((wrapper) => {
        ...
        function updateActiveClasses(targetIdx, hideContent = false) {
            const index = targetIdx !== undefined ? targetIdx : currentSlide;
            slides.forEach(slide => slide.classList.remove("active"))

            if (!hideContent && index >= 0 && index < totalSlides) {
                slides[index].classList.add("active")
            }

            let realIndex = index - 1
            if (index === 0) realIndex = visibleSlides - 1
            if (index === totalSlides - 1) realIndex = 0
        ...
        function goToSlide(targetIndex) {
            ...
            // Esconde el contenido actual y actualiza bullets
            updateActiveClasses(targetIndex, true)

            // Espera a que termine la animación de salida del texto
            setTimeout(() => {
