 ...
 *
 * This file serves as the main engine for the Avante theme, encapsulating
...
function setup_avante()
{
    ...
    register_nav_menus(
        array(
            'primary' => __('Main Menu', 'avante'),
            'social' => __('Social Menu', 'avante'),
            'footer-1' => __('Footer 1', 'avante'),
            'footer-2' => __('Footer 2', 'avante'),
            'footer-3' => __('Footer 3', 'avante'),
        )
    );
...
add_action('after_setup_theme', 'setup_avante');
 ...
 */
function avante_override_theme_json_colors($theme_json)
{
    ...
    if (!function_exists('avante_get_color_themes')) {
        return $theme_json;
    }
    ...
    foreach (['post-shadow-light', 'post-shadow-light-hover', 'post-shadow', 'post-shadow-hover', 'button-shadow', 'button-shadow-hover'] as $id) {
...
add_filter('wp_theme_json_data_theme', 'avante_override_theme_json_colors');
...
function avante_get_assets()
{
    ...
    return [
        'css' => [
            ...
            'crisisacademy-crisis-simulator' => "$assets_path/css/crisisacademy-homepage/crisis-simulator.css",
        ...
        'js' => [
            ...
            'ws-script' => "$assets_path/js/crisisacademy-homepage/ws-script.js",
        ]
    ];
}
...
function footer_components()
{
    ...
    wp_localize_script('global-script', 'avante_likes_obj', [
...
function add_gtm_header()
{
    ...
    $ga_id = get_option('avante_ga_id');
    
    if (false === $ga_id || empty($ga_id)) {
        $ga_id = get_theme_mod('avante_ga_id', $default);
    }
...
{
    $separator = avante_get_icon('separator');
    $icon_home = avante_get_icon('home');
    $home = 'Inicio';
    $showCurrent = 1;
    $showOnHome = 0;
    $current = '';
    ...
    elseif (is_post_type_archive('nsfw')) {
        if ($paged === 1) {
            ...
            echo '<h1 class="page-title">' . esc_html__('Contenido NSFW', 'avante') . '</h1>';
        } else {
            echo '<span>' . esc_html__('Página ', 'avante') . $paged . esc_html__(' de ', 'avante') . '</span>' . '<h1 class="page-title">' . esc_html__('todo el contenido NSFW', 'avante') . '</h1>';
        }
    }
...
function avante_get_icon($type)
{
    $icons = [
        ...
        'star' => '<svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>',

        // Crisis Academy
        'grid' => '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-columns" viewBox="0 0 16 16"><path d="M0 2a1 1 0 0 1 1-1h14a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1zm8.5 0v8H15V2zm0 9v3H15v-3zm-1-9H1v3h6.5zM1 14h6.5V6H1z"/></svg>',
        'graph-down-arrow' => '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-graph-down-arrow" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M0 0h1v15h15v1H0zm10 11.5a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 .5-.5v-4a.5.5 0 0 0-1 0v2.6l-3.613-4.417a.5.5 0 0 0-.74-.037L7.06 8.233 3.404 3.206a.5.5 0 0 0-.808.588l4 5.5a.5.5 0 0 0 .758.06l2.609-2.61L13.445 11H10.5a.5.5 0 0 0-.5.5"/></svg>',
...
function custom_menu($item_output, $item, $depth, $args)
{
    ...
    if ($has_children) {
        ...
        $svg_icon = avante_get_icon('chevron-down');
...
function custom_render_gallery_block($block_content, $block)
{
    ...
    <div class="post-gallery-wrapper">
        ...
        <div class="post-gallery-thumbs-container">
            <button class="btn-pagination" aria-label="Anterior"><?= avante_get_icon('backward'); ?></button>
            <div class="post-gallery-thumbs"></div>
            <button class="btn-pagination" aria-label="Siguiente"><?= avante_get_icon('forward'); ?></button>
        </div>
    </div>
...
function custom_wp_block_search($block_content, $block)
{
    if ($block['blockName'] === 'core/search') {
        ob_start();
        ?>
        <form role="search" method="get" action="<?php echo home_url('/'); ?>" class="avante-block-search">
            <div class="avante-block-search__inside-wrapper ">
                <input class="avante-block-search__input" id="avante-block-search__input-<?php echo $block['clientId']; ?>"
                    placeholder="<?php esc_html_e('Buscar', 'avante'); ?>" value="" type="search" name="s" required="">
                <button aria-label="<?php esc_html_e('Buscar', 'avante'); ?>"
                    class="avante-block-search__button avante-element-button" type="submit">
...
function custom_category_list($output, $args)
{
    ...
    $output = '';
    foreach ($categories as $category) {
        $svg_icon = avante_get_icon('category');
 ...
 */
function avante_has_related_posts($post_id = null)
{
    if (!$post_id) {
        $post_id = get_the_ID();
    }
...
function page_template()
{
    ...
    if (is_page() or is_single()) {
        ...
        if (is_single()) {
            avante_enqueue_style('single', $a['css']['single']);
            if (avante_has_related_posts()) {
            ...
            if (comments_open()) {
                avante_enqueue_style('custom-comments', $a['css']['comments']);
            }
        }
    }
}
...
function page404_styles()
{
    if (is_404()) {
        $a = avante_get_assets();
        avante_enqueue_style('error404', $a['css']['error404']);
    }
}
 ...
 *
 * It uses custom enqueue helpers (avante_enqueue_style/script)
...
function properties_templates() {
    if ( is_page_template( 'archive-property.php' ) ) {
        ...
        avante_enqueue_script( 'reset', $a['js']['reset-properties-filter'] );
        avante_enqueue_script( 'ajax-search-from-other-page', $a['js']['ajax-search'] );
        avante_enqueue_script( 'ajax-properties', $a['js']['ajax-properties'] );
...
function participants_templates() {
    ...
    if ( is_page_template('templates/ca-homepage.php') ) {
        $a = avante_get_assets();
 ...
 */
function avante_nsfw_archive_query($query)
{
...
add_action('wp_enqueue_scripts', 'avante_enqueue_home_assets');

/**
 * AJAX Handler for filtering posts
 */
function avante_filter_posts_handler() {
    // 1. Verificar seguridad
    // Para usuarios no logueados somos más flexibles para evitar fallos por cache de página
    $nonce = isset($_POST['nonce']) ? $_POST['nonce'] : '';
    $nonce_valido = wp_verify_nonce($nonce, 'avante_home_nonce');
