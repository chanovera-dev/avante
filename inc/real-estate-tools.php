...
function eb_get_properties($operation_type = null, $limit = 12) {
    $keys = function_exists('avante_get_eb_api_keys') ? avante_get_eb_api_keys() : [];
...
function get_existing_property_types() {
    ...
    foreach ($raw_results as $raw_type) {
        if (!empty($raw_type)) {
            $normalized = avante_get_normalized_property_type($raw_type);
...
add_filter('manage_property_posts_columns', function($columns) {
    $columns['featured'] = __('Featured', 'avante');
    return $columns;
});
 ...
 */
function avante_render_metadata_item($type, $value, $args = []) {
    // Defaults
    $defaults = [
        'unit' => '',
        'class' => '',
 ...
 */
function avante_display_property_metadata($post_id = null, $args = []) {
    if (!$post_id) {
        $post_id = get_the_ID();
    }
    ...
    foreach ($metadata as $key => $meta) {
        ...
        // Get icon
        $icon = avante_get_icon($meta['type']);
 ...
 */
function avante_get_full_property_metadata($post_id = 0) {
    if (!$post_id) {
        $post_id = get_the_ID();
    }
...
function avante_render_full_property_metadata($post_id = 0) {
    ...
    $metadata = avante_get_full_property_metadata($post_id);
    
    // ID
    echo '<li>';
    echo '<span>' . avante_get_icon('id') . '</span> ';
    echo 'ID: ' . esc_html($metadata['id']);
    echo '</li>';
    
    // Location
    echo '<li>';
    echo '<span>' . avante_get_icon('location') . '</span> ';
    ...
    if (!empty($metadata['bedrooms']) && $metadata['bedrooms'] != 0) {
        ...
        echo '<span>' . avante_get_icon('bedroom') . '</span> ';
    ...
    if (!empty($metadata['bathrooms']) && $metadata['bathrooms'] != 0) {
        echo '<li>';
        echo '<span>' . avante_get_icon('bathroom') . '</span> ';
    ...
    if (!empty($metadata['parking']) && $metadata['parking'] != 0) {
        ...
        echo '<span>' . avante_get_icon('parking') . '</span> ';
    ...
    if (!empty($metadata['construction']) && $metadata['construction'] != 0) {
        echo '<li>';
        echo '<span>' . avante_get_icon('construction') . '</span> ';
    ...
    if (!empty($metadata['lot']) && $metadata['lot'] != 0) {
        echo '<li class="lot">';
        echo '<span>' . avante_get_icon('lot') . '</span> ';
 ...
 */
function avante_register_property_acf_fields() {
...
add_action('acf/init', 'avante_register_property_acf_fields');
 ...
 */
function avante_convert_acf_gallery_to_eb_format($post_id) {
    ...
    // The avante_get_property_data() function handles the combination.
}

add_action('acf/save_post', 'avante_convert_acf_gallery_to_eb_format', 20);
 ...
 */
function avante_get_normalized_property_type($type) {
 ...
 */
function avante_get_normalized_operation($operation) {
 ...
 * @param int $post_id Post ID
 */
function avante_fix_property_select_values($post_id) {
    ...
    if (!empty($property_type)) {
        $normalized_type = avante_get_normalized_property_type($property_type);
    ...
    $operation = get_post_meta($post_id, 'eb_operation', true);
    if (!empty($operation)) {
        $normalized_op = avante_get_normalized_operation($operation);
...
// Hook right after ACF saves (priority 15 to run before avante_process_manual_property_save)
add_action('acf/save_post', 'avante_fix_property_select_values', 15);
 ...
 * @param int $post_id Post ID
 */
function avante_process_manual_property_save($post_id) {
    // Only for property post type
    if (get_post_type($post_id) !== 'property') {
        return;
    }
...
add_action('acf/save_post', 'avante_process_manual_property_save', 25);
 ...
 * @return array
 */
function avante_get_eb_api_keys() {
    $keys = get_option('avante_eb_api_keys');
...
function avante_register_property_admin_pages() {
    ...
    add_submenu_page(
        ...
        __('Dashboard de Propiedades', 'avante'),
        __('Dashboard', 'avante'),
        'manage_options',
        'property-dashboard',
        'avante_property_dashboard_render'
    );

    // Configuration
    add_submenu_page(
        'edit.php?post_type=property',
        __('Configuración de EasyBroker', 'avante'),
        __('Configuración', 'avante'),
        'manage_options',
        'property-settings',
        'avante_property_settings_render'
    );
}
add_action('admin_menu', 'avante_register_property_admin_pages');

/**
 * Enqueue styles for the Property Dashboard and Settings.
 */
function avante_property_dashboard_assets($hook) {
...
add_action('admin_enqueue_scripts', 'avante_property_dashboard_assets');
...
add_action('admin_init', function() {
    ...
    if (get_option('avante_eb_keys_migrated')) {
        return;
    }

    // Migrate existing options if they contain non-default values
    $k1 = get_option('avante_eb_api_key');
    $k2 = get_option('avante_eb_api_key_2');
    ...
    if (!empty($initial_keys)) {
        update_option('avante_eb_api_keys', array_values(array_unique($initial_keys)));
    }
    
    // Mark as migrated
    update_option('avante_eb_keys_migrated', true);
});

/**
 * Render the Property Settings page.
 */
function avante_property_settings_render() {
    // Handle form submission
    if (isset($_POST['avante_save_settings']) && check_admin_referer('avante_eb_settings_nonce')) {
        if (!current_user_can('manage_options')) {
        ...
        update_option('avante_eb_api_key', $cleaned_keys[0] ?? '');
        update_option('avante_eb_api_key_2', $cleaned_keys[1] ?? '');
    ...
    if (empty($keys)) $keys = [''];
    ?>
    <div class="wrap avante-admin-wrap">
        <h1 class="wp-heading-inline"><?php _e('Configuración de EasyBroker', 'avante'); ?></h1>
        <hr class="wp-header-end">

        <div class="avante-dashboard-grid secondary">
            <div class="avante-card full-height">
                <h3><?php _e('API Keys de EasyBroker', 'avante'); ?></h3>
                <p><?php _e('Gestiona tus claves API para la sincronización de propiedades. Puedes agregar múltiples cuentas si lo necesitas.', 'avante'); ?></p>
                
                <form method="post" action="">
                    <?php wp_nonce_field('avante_eb_settings_nonce'); ?>
                    ...
                    <div id="eb-keys-container">
                        <?php foreach ($keys as $index => $key_value): ?>
                            <div class="eb-key-row" style="display: flex; gap: 10px; align-items: center; margin-bottom: 15px;">
                                <input type="text" name="eb_api_keys[]" value="<?php echo esc_attr($key_value); ?>" class="regular-text" style="flex: 1; max-width: 500px;" placeholder="<?php esc_attr_e('Ingrese su API Key', 'avante'); ?>">
                                <button type="button" class="button remove-key" style="color: #d63638; border-color: #d63638;"><?php _e('Eliminar', 'avante'); ?></button>
                            </div>
                        <?php endforeach; ?>
                    </div>

                    <button type="button" id="add-eb-key" class="button button-secondary" style="margin-bottom: 30px; margin-top: 10px;">
                        <span class="dashicons dashicons-plus" style="margin-top: 4px;"></span> <?php _e('Agregar otra clave API', 'avante'); ?>
                    </button>
 ...
 */
function avante_get_property_stats() {
    ...
    foreach ($raw_types as $rt) {
        $normalized = function_exists('avante_get_normalized_property_type') ? avante_get_normalized_property_type($rt->meta_value) : $rt->meta_value;
 ...
 */
function avante_property_dashboard_render() {
    $stats = avante_get_property_stats();
    $last_sync = get_option('eb_last_sync_time', __('Nunca', 'avante'));
    ?>
    <div class="wrap avante-admin-wrap">
        <h1 class="wp-heading-inline"><?php _e('Dashboard de Propiedades', 'avante'); ?></h1>
        <hr class="wp-header-end">

        <div class="avante-dashboard-grid">
            
            <!-- Summary Cards -->
            <div class="avante-card summary-card">
                <div class="card-icon dashicons dashicons-admin-home"></div>
                <div class="card-content">
                    <h3><?php _e('Total Propiedades', 'avante'); ?></h3>
                    <span class="card-value"><?php echo esc_html($stats['total']); ?></span>
                    <p class="card-subtitle"><?php printf(__('%d publicadas, %d borradores', 'avante'), $stats['published'], $stats['draft']); ?></p>
                </div>
            </div>

            <div class="avante-card operation-card">
                <div class="card-icon dashicons dashicons-tag"></div>
                <div class="card-content">
                    <h3><?php _e('En Venta', 'avante'); ?></h3>
        ...
        <div class="avante-dashboard-grid secondary">
            ...
            <!-- Types Table -->
            <div class="avante-card full-height">
                <h3><?php _e('Propiedades por Tipo', 'avante'); ?></h3>
                <ul class="stats-list">
            ...
            <!-- Locations Table -->
            <div class="avante-card full-height">
                <h3><?php _e('Principales Ciudades', 'avante'); ?></h3>
                <ul class="stats-list dots">
                    <?php foreach ($stats['locations'] as $city => $count): ?>
                        <li>
                            <span class="label"><?php echo esc_html($city); ?></span>
                            <span class="count"><?php echo esc_html($count); ?> <?php _e('propiedades', 'avante'); ?></span>
                        </li>
                    <?php endforeach; ?>
                </ul>
            </div>
