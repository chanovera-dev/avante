...
function eb_sync_properties() {
    ...
    $api_keys = function_exists('avante_get_eb_api_keys') ? avante_get_eb_api_keys() : [];
    ...
    foreach ( $api_keys as $api_key ) {
        ...
        do {
            ...
            foreach ( $properties as $p ) {
                ...
                update_post_meta($post_id, 'eb_operation', avante_get_normalized_operation($operation_type));
                update_post_meta($post_id, 'eb_currency', $currency);
                update_post_meta($post_id, 'eb_location', eb_safe_value($p, 'location', 'No location'));
                update_post_meta($post_id, 'eb_property_type', avante_get_normalized_property_type(eb_safe_value($p, 'property_type', 'No type')));
